...
The `<Toaster />` automatically flips `left`↔`right` positions when inside an RTL `<DirectionProvider />`. `mobilePosition` is unaffected since it only controls the top/bottom edge.
